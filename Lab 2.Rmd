...
At first glance from the output above, it seems that flight number 677 uniquely represents the flight from JFK to MIA by carrier AA. (Fill in the Blanks). 
...
Why or why not? Because different carriers will use the same flight number to denote a different flight path (origin->destination)

If we know the flight number, carrier, and origin, can we know for sure what the destination will be? Yes (*Yes/No*)

Why or why not? A flight number for a particular carrier describes an origin to a destination. If there was a different destination, there would be a different flight number.
...
A negative gain_hr indicates that time was made up on the flight in spite of the delay, and a positive gain_hr indicates the flight took more time than intended. A gain_hr of 0 indicates that arr_delay_hr is equal to dep_delay_hr, meaning the flight took the expected amount of time to arrive. 
...
# Wasn't part of the assignment thus far, but there is an issue with the crossover from 11:59pm to 12:00am as can be seen with these exceptionally high dep_delay_hrs
```
...
```{r,eval=F}
print(identical(f2e,f2e.pipedream))
```
...
# # alternative accuracy maybe?
# f.accuracy <- na.omit(f.accuracy)
# min_accuracy <- min(f.accuracy$accuracy)
# max_accuracy <- max(f.accuracy$accuracy)
# f.accuracy$normalized_accuracy <- (1 - ((f.accuracy$accuracy - min_accuracy) / (max_accuracy - min_accuracy)))^2
