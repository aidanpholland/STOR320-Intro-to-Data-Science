...
for (row in 1:nrow(OUT.MAE)) {
  for (col in 1:ncol(OUT.MAE)) {
    DATA3 <- DATA2 %>%
      mutate(training=map(train,train.model.func,I=row,J=col)) %>%
      mutate(predict=map2(test,training,~augment(.y,newdata=.x))) %>%
      select(predict) %>%
      unnest()
...
BEST3.DATA2 <- BEST3.DATA %>% 
            gather('First','Second','Third',key="Model",value="Predict",factor_key=T)
...
```{r,eval=T}
ggplot(BEST3.DATA2[BEST3.DATA2$L==920,]) +
  geom_point(aes(x=D,y=W),alpha=0.05,stroke=0) + 
  geom_line(aes(x=D,y=Predict,color=Model)) +
  xlab("Julian Day")+
  ylab("Max Water Temperature")+
  ggtitle("Location 920 Data")
```

Answer *(2 Points)*: The top three models are nearly indistinguishable from one another, indicating that they all predict around the same max water temperatures for the data.
...
Answer: For $I$, I would choose 4, and for $J$, I would choose 6. On each plot, the number corresponds to just about the minimum avgMAE. The graphics are telling us that more data on Max Water Temperature and Julian Day is not necessarily beneficial for minimizing avgMAE as the graph of I loses accuracy as I increases and J seems to simply reach an asymptote, at least on the scale that we are observing.
...
Code and Output *(3 Points)*:
```{r,warning=F}
trainingmodel=function(data,I,J) {
  model=lm(W~poly(A,I)+poly(D,J)+I(A*D),data=data)
  return(model)
}
DATA3 <- DATA2 %>%
  mutate(training=map(train,trainingmodel,I=2,J=6)) %>%
  mutate(predicting=map2(test,training,~augment(.y,newdata=.x))) %>%
  select(predicting) %>%
  unnest()
MAE.func(actual=DATA3$W,predict=DATA3$.fitted)
```
...
```{r,warning=F}
trainingmodel=function(data,I,J) {
  model=lm(W~poly(A,I)+poly(D,J)+I(A*D)+L,data=data)
  return(model)
}
DATA3 <- DATA2 %>%
  mutate(training=map(train,trainingmodel,I=2,J=6)) %>%
...
Code and Output:
```{r}
bestpolymodelint <- lm(W~poly(A,2)+poly(D,6)+I(A*D),data=BEST3.DATA2)
bestpolymodell <- lm(W~poly(A,2)+poly(D,6)+L,data=BEST3.DATA2)
BEST3.DATA2$"Best Poly + Interact" <- predict(bestpolymodelint,newdata=BEST3.DATA2)
BEST3.DATA2$"Best Poly + L" <- predict(bestpolymodell,newdata=BEST3.DATA2)
predicts <- BEST3.DATA2 %>%
  gather(Model,Predict,c("Predict","Best Poly + Interact","Best Poly + L"))

ggplot(BEST3.DATA2[BEST3.DATA2$L==920,]) +
  geom_point(aes(x=D,y=W),alpha=0.05,stroke=0) + 
  geom_line(aes(x=D,y=Predict,color=Model)) +
  scale_color_manual(values=c("blue","red","green"),labels=c("Best Poly","Best Poly + Interact","Best Poly + L")) +
  xlab("Julian Day") +
  ylab("Max Water Temperature")
```