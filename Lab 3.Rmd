...
flight.summary = 
  ...
  mutate(
    ...
    accuracy      = abs(arr_delay_min)+abs(dep_delay_min)
  ) %>%
  
  #3
  filter(!is.na(dep_delay_min) & !is.na(arr_delay_min)) %>%
...
Accuracy is a value that we want to minimize, as a higher accuracy value indicates that the delays of arrival or departure are high. JFK clearly has the highest values in the plot, with some grouped origin,dest,carrier combinations having an accuracy score above 600 minutes, or 10 hours. JFK's trend line also indicates that an increasing mean distance of a origin,dest,carrier results in a slightly increased mean accuracy. Therefore, JFK is the worst originating airport.
...
```{r, eval=T}
kable(flight.summary2, format = "html") %>%
...
1) Is there a particular carrier that operates out of JFK that I should avoid, given that I must use JFK? That is to say, does a particular carrier out of JFK have a particularly bad track record?

2) None of the data we've worked with involves price. It may be a difficult variable to work with, as complex algorithms depending on time of year, proximity to the date of flight, and other factors cause that value to change a lot. However, I would ask: how do accuracy and price associate for the different origin,dest combinations? Is there a pattern with price of certain carriers when considering the same origin,dest combinations?