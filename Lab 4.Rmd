...
final.tidy.school = untidy.school %>%
                      ...
                      spread(key=type, value=Value) %>%
                      complete(school, student, Year) %>%
                      mutate(CID = paste(school, student, sep = ".")) %>%
                      select(school,student,CID,Year,Percentile,Score)
...
The superintendent should distribute the money evenly among the schools because the variation in the median is relatively consistent within the two years, indicating that improvement across the board occurred and that if the schools were ranked on their improvement, the first school likely would not have improved that much more than the second.
...
If we're talking the percentiles on just a county or small area in the Cleveland area, as opposed to national percentiles, this makes perfect sense. If the percentiles were originally only pertaining to that small area, then regardless of almost every single student improving, the percentiles will only rearrange themselves. The bottom percentile of student may have improved their math game, but still landed in under the 10th percentile because other students improved more or had higher grades. Most of the students that were originally in the highest percentile declined, barring one or two people that maintained their percentile.
...
Given that the metric for improvement is performance on a standardized test (in my mind, an SAT of sorts that is conducted on a national scale), I see no potential for fowl play in the way of making the test easier so that scores improve. Call me too trusting, it seems as though almost every student in the Cleveland area that was polled improved. However, as the beginning of the lab states, extra, and definitely excessive, time was spent on math in order to secure the money. The students were probably instructed in a way that perfectly prepares them for the standardized test, but possibly results in an overfitting issue in which students are only prepared for that math test and no other mathematical benchmark, not to mention the disregard for other subjects in favor of math. As aforementioned, the percentiles were possibly only rearranged top to bottom given the polling area only being applied to itself and not a national scale. So yes, I believe the math knowledge improved.