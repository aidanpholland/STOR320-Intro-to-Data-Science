...
delay = flights %>%
  ...
  select(dest,avg_arr_delay,name,lat,lon)
delay #Do Not Remove
```
...
flights %>%
  ...
  ggplot(aes(x=avg_dep_delay,y=age)) +
  geom_point()
```
...
```
Answer: It means that there was no departure or arrival data except for what was scheduled, indicating that the flight likely was cancelled.
...
Answer: The tail numbers that don't have matching records are the planes that are reported as fleet numbers rather than tail numbers.