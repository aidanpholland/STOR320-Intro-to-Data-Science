...
We add 1 to log transformed variables because attempting to take the log of zero results in an undefined answer.

**Question:** In complete sentences, what is the purpose of adding 1 in the creation of the year variable?

We add 1 because we want the duration to include the current year.
...
```{r,eval=T}
votesgeek <- ggplot(bgg3,aes(x=num_votes,y=geek_rating)) +
  geom_point() +
  labs(x="Number of Votes",y="Geek Rating") +
  ggtitle("Geek Rating vs Number of Votes")
ownedgeek <- ggplot(bgg3,aes(x=owned,y=geek_rating)) +
  geom_point() +
  labs(x="Owned",y="Geek Rating") +
  ggtitle("Geek Rating vs Ownership")
logvotesgeek <- ggplot(bgg3,aes(x=log_vote,y=geek_rating)) +
  geom_point() +
  labs(x="Log(Number of Votes + 1)",y="Geek Rating") +
  ggtitle("Geek Rating vs Log(Number of Votes + 1)")
logownedgeek <- ggplot(bgg3,aes(x=log_own,y=geek_rating)) +
  geom_point() +
  labs(x="Log(Ownership + 1)",y="Geek Rating") +
  ggtitle("Geek Rating vs Log(Ownership + 1)")
votesgeek
ownedgeek
logvotesgeek
logownedgeek
...
The relationship changes when you take the log of the ind. variable in that the graph appears more linear.
...
bgg4 <- bgg3 %>%
  mutate(Set=sample(c("Train","Test"),size=n(),replace=T,prob=c(0.8, 0.2)))
...
lm3 (third model) does the best job at predicting the geek rating of the board games because its MAE is the lowest of the 3 options.
...
```{r,eval=T}
crossv <- crossv_kfold(data=bgg4, k=10)
models <- map(crossv$train,~lm3)
mean(map2_dbl(models,crossv$test,mae))
```
...
The absolute differece b/t the out-of-sample mean absolute error measured using a test set and the mean absolute error measured using cross validation can be calculated using this formula: abs(MAE.func(test.bgg2$lm3_resid)-mean(map2_dbl(models,crossv$test,mae))), which is equal to 0.004334289

```{r,eval=T}
abs_diff <- abs(MAE.func(test.bgg2$lm3_resid)-mean(map2_dbl(models,crossv$test,mae)))
abs_diff
```